...
This is the implementation-side counterpart to the verification-side audit protocol at `C:\Users\scott\Documents\antigravity\eager-archimedes\PRINTERPROOF3D_AUDIT_PROTOCOL.md`. The verification protocol governs how the auditing agent audits work that has already landed. This document governs how the implementing agent audits its own work *before* a push, so the verification turn finds less to fix.

Both Antigravity and Antigravity read this file. The rule body, the artifact-state checklist, and the report format below are shared. The implementing-agent-side discipline (chat-promise rejection) and the verifier-side discipline (mandatory 10-section output) live in their respective files.
